<template>
  ...
  <div class="modal fade" id="editModalContent" tabindex="-1" role="dialog" aria-labelledby="editModalContent" aria-hidden="true">
      <div class="modal-dialog" role="document">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" id="verifyModalContent_title">EDIT COUNTRY</h5>
                  <button class="btn btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <Form @submit="editCountry" :validation-schema="schema" class="user">
                  <div class="modal-body">
                          ...
                          <div class="form-group">
                              <label class="col-form-label" for="country_name">Country Name:</label>
                              <Field name="country_name" class="form-control" v-model="country_name" type="text" />
                              <ErrorMessage name="country_name" class="text-danger p-3" />
                          </div>
                  </div>
                  <div class="modal-footer">
      ...
      <div class="main-content-wrap d-flex flex-column">
        <div class="main-content">
          <button class="btn btn-info text-white ul-btn-raised--v2 m-1 text-white float-end" type="button" data-bs-toggle="modal" data-target="#verifyModalContent" data-whatever="@mdo">
                          <i class="nav-icon i-add text-white fw-bold"></i> ADD COUNTRY</button>
            <Breadcrumbs />
                      ...
                      <table class="table" id="country_table">
                        ...
                        <tbody>
                          <tr v-for="(country, index) in countries" v-bind:key="country._id">
                            ...
                            <td>
                              <a class="text-info me-2" @click="openEditCountry(country)"><i class="nav-icon i-Pen-2 fw-bold"></i></a>
                              <a class="text-danger me-2" @click="deleteCountry(country._id)"><i class="nav-icon i-Close-Window fw-bold"></i></a>
                            </td>
                          </tr>
                        </tbody>
                      </table>
  ...
  import { ALL_COUNTRIES_QUERY, ADD_COUNTRY_MUTATION, EDIT_COUNTRY_MUTATION, DELETE_COUNTRY_MUTATION } from '@/graphql'
  export default {
    name: "Country",
