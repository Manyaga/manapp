<template>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="verifyModalContent_title">ADD INTEREST CATEGORY</h5>
                    <button class="btn btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <Form @submit="addCategory" :validation-schema="schema" class="user">
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="col-form-label" for="name">Interest Category:</label>
    ...
    <div class="modal fade" id="editModalContent" tabindex="-1" role="dialog" aria-labelledby="editModalContent"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalContent_title">EDIT INTEREST CATEGORY</h5>
                    <button class="btn btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <Form @submit="editCountry" :validation-schema="schema" class="user">
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="col-form-label" for="name">Interest Category:</label>
                            <Field name="name" class="form-control" v-model="name"
                                @input="remark = $event.target.value.toUpperCase()" type="text" />
                            <ErrorMessage name="name" class="text-danger p-3" />
                        </div>
                    </div>
                    <div class="modal-footer">
        ...
        <div class="main-content-wrap d-flex flex-column">
            <div class="main-content">
                <button class="btn btn-info text-white ul-btn-raised--v2 m-1 text-white float-end" type="button"
                    ...
                    <i class="nav-icon i-add text-white fw-bold"></i> ADD INTEREST CATEGORY
                </button>
                <Breadcrumbs />
                ...
                <div class="row mb-4">
                    <div class="col-md-12">
                        <table class="table" id="category_table">
                            ...
                            <tbody>
                                <tr v-for="(category, index) in InterestCategories" v-bind:key="category._id">
                                    <td>{{ index + 1 }}</td>
                                    <td> {{ category.name.toUpperCase() }}</td>
                                    <td> <img class="rounded-circle m-0 avatar-sm-table"
                                            src="../../assets/images/faces/1.jpg" alt="" /></td>
                                    <td>
                                        <a class="text-info me-2" @click="openEditCategory(category)"><i
                                                class="nav-icon i-Pen-2 fw-bold"></i></a>
                                        <a class="text-danger me-2" @click="deleteCategory(category._id)"><i
...
import { ALL_INTEREST_CATEGORIES_QUERY, ADD_INTEREST_CATEGORY_MUTATION, EDIT_INTEREST_CATEGORY_MUTATION, DELETE_INTEREST_CATEGORY_MUTATION } from '@/graphql'
export default {
    name: "Category",
    ...
    data() {
        const schema = yup.object().shape({
            category: yup
                .string()
                .required("Interest Categoryname is required!"),
            category_id: yup
        ...
        addCategory(category) {
            this.$apollo
                ...
                .then(response => {
                    ...
                    this.$swal({
                        title: 'Interest Categoryadded sucessfully',
                        position: 'top-end',
                        icon: 'success',
        ...
        editCountry(category) {
            ...
            this.$apollo
                ...
                .then(response => {
                    ...
                    this.$swal({
                        title: 'Interest Categoryupdated sucessfully',
                        position: 'top-end',
                        icon: 'success',
                        showConfirmButton: false,
                        timer: 2000
            ...
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.$apollo
                        ...
                        .then(response => {
                            this.$swal({
                                title: 'Interest Categorydeleated sucessfully',
                                position: 'top-end',
                                icon: 'success',
                                showConfirmButton: false,
                                timer: 2000
