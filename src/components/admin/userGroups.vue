<template>
  ...
  <div class="modal fade" id="editModalContent" tabindex="-1" role="dialog" aria-labelledby="editModalContent"
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="editModalContent_title">Edit Group {{ group_name }}</h5>
          <button class="btn btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        ...
        <div class="row mb-4">
          <div class="col-md-12">
            <table class="table" id="group_table">
              ...
              <tbody>
                <tr v-for="(group, index) in userGroups" v-bind:key="group.group_id">
                  <td>{{ index + 1 }}</td>
                  <td> {{ group.group_name.toUpperCase() }}</td>
                  <td>
                    <a class="text-info me-2" @click="openEditCounty(group)"><i class="nav-icon i-Pen-2 fw-bold"></i></a>
                    <a class="text-danger me-2" @click="deleteCounty(group.group_id)"><i
