<template>
  ...
  <div class="modal fade" id="editModalContent" tabindex="-1" role="dialog" aria-labelledby="editModalContent"
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="verifyModalContent_title">Edit Service</h5>
          <button class="btn btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        ...
        <div class="row mb-2">
          <div class="col-md-12">
            <button class="btn btn-light text-white ul-btn-raised--v2 m-1  float-end" type="button" data-bs-toggle="modal"
              ...
              <i class="nav-icon i-add text-primary text-white fw-bold"></i> ADD SERVICE</button>
          </div>
        </div>
        <div class="row mb-4">
          <div class="col-md-12">
              ...
              <table class="table text-center" id="service_table">
                ...
                <tbody>
                  <tr v-for="service, index in services" :key="service.service_id">
                    ...
                    <td>
                      <a class="text-success me-2" @click="openEditService(service)"><i
                          class="nav-icon i-Pen-2 fw-bold"></i></a>
                      <a class="text-danger me-2 float-end" @click="deleteService(service.service_id)"><i
...
import { ALL_SERVICES_QUERY, ADD_SERVICE_MUTATION, DELETE_SERVICE_MUTATION, EDIT_SERVICE_MUTATION } from '@/graphql';
